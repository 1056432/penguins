...
```{r, include=FALSE}
#This file contains the functions needed to carry out this analysis:
source("functions/libraries.r") 
```

The working directory will be different for every user. 
Set the directory to where the PenguinProject folder is saved on your computer. 
```{r}
#setwd(insert file path here)

setwd("C:/Users/ashaw/Dropbox/Asha/Uni/Third Year/Computer skills/Session 5- Reproducible research in R and making figures/PenguinProject")
```
...
```
This scatterplot supports that there is no significant interaction between penguin species and body mass because the regression lines have a roughly equal gradient.   
